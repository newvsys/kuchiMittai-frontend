  ...
  const updateOrder = async () => {
    if (
      ...
      if (!isValidNameOrLastname(order?.name)) {
        showError("You entered invalid name format");
        return;
      }

      if (!isValidNameOrLastname(order?.lastname)) {
        showError("You entered invalid lastname format");
        return;
      }

      if (!isValidEmailAddressFormat(order?.email)) {
        showError("You entered invalid email format");
        return;
      }
      ...
      apiClient.put(`/api/orders/${order?.id}`, {
        ...
        .then((response) => {
          if (response.status === 200) {
            showToast("Order updated successfuly");
          } else {
            throw Error("There was an error while updating a order");
          }
        })
        .catch((error) =>
          showError("There was an error while updating a order")
        );
    } else {
