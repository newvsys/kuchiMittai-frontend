...
const DashboardCreateNewUser = () => {
  ...
  const addNewUser = async () => {
    if (userInput.email === "" || userInput.password === "") {
      showError("You must enter all input values to add a user");
      return;
    }
    ...
    if (
      ...
      if (!isValidEmailAddressFormat(userInput.email)) {
        showError("You entered invalid email address format");
        return;
      }
      ...
      if (userInput.password.length > 7) {
        ...
        showError("Password must be longer than 7 characters");
      }
    } else {
      showError("You must enter all input values to add a user");
    }
