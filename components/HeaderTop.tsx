...
const HeaderTop = () => {
  ...
  const handleLogout = () => {
    ...
    router.push("/search?price=10000&minPrice=0&inStock=false&sort=defaultSort&page=1");
      ...
      <div className="flex items-center justify-between max-lg:flex-wrap gap-x-4 gap-y-2 max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-10 pt-0 pb-0">
        ...
        <ul className="flex items-center gap-x-5 max-[370px]:text-sm max-[370px]:gap-x-2 font-semibold">
          {isAdmin ? (
            ...
            <>
              <li>
                <Link href="/admin/categories" className="flex items-center gap-x-2 text-sm font-semibold bg-blue-600 text-white px-3.5 py-2 rounded hover:bg-blue-700 transition-colors shadow-sm whitespace-nowrap">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 7h18M3 12h18M3 17h18" /></svg>
                  <span>Admin Dashboard</span>
                </Link>
              </li>
              <li><NotificationBell /></li>
              <li>
                <button onClick={handleLogout} className="flex items-center gap-x-2 text-sm font-semibold bg-gray-100 text-gray-700 px-3.5 py-2 rounded hover:bg-gray-200 transition-colors shadow-sm whitespace-nowrap">
